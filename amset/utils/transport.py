...
def calculate_sio(tp, c, temperature, ib, ik, once_called, kgrid, cbm_vbm,
    ...
    f_th = kgrid[tp]["f_th"][c][temperature][ib][ik]
    ...
    for j, X_Epm in enumerate(["X_Eplus_ik", "X_Eminus_ik"]):
        ...
        for X_ib_ik in kgrid[tp][X_Epm][ib][ik]:
            ...
            g_pm_th = kgrid[tp]["g_th"][c][temperature][ib_pm][ik_pm]
            ...
            if tp == "n":
                ...
                f_pm_th = kgrid[tp]["f_th"][c][temperature][ib_pm][ik_pm]
            else:
                f_pm = 1 - kgrid[tp]["f"][c][temperature][ib_pm][ik_pm]
                f_pm_th = 1 - kgrid[tp]["f_th"][c][temperature][ib_pm][ik_pm]
            ...
            s_i_th[j] += (n_pop + (1 - j) + (-1) ** (
                    1 - j) * f_th) * lamb_ipm * g_pm_th
        if counted > 0:
            s_i[j] /= counted
            s_i_th[j] /= counted
            s_o[j] /= counted
