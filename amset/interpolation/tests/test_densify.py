...
amset_files = os.path.join(test_dir, "..", "..", "..", "examples", "Si")
...
class TestPeriodicVoronoi(unittest.TestCase):
    ...
    def test_densify(self):
        ...
        vr = Vasprun(
            os.path.join(amset_files, "vasprun.xml.gz"), parse_projected_eigen=True
        )
        ...
        amset_data.set_doping_and_temperatures(
            doping=np.array([1e13]), temperatures=np.array([300])
        )
        amset_data.calculate_fd_cutoffs(fd_tolerance=0.000001)

        densifier = BandDensifier(inter, amset_data, energy_cutoff=2, bandgap=1.33)
