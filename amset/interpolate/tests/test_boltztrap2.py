...
class TestBoltzTraP1Interpolater(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.correct_dos = loadfn(
            os.path.join(test_dir, "dos.json.gz"))
        self.correct_norm_dos = loadfn(
            os.path.join(test_dir, "norm_dos.json.gz"))
    ...
    def test_get_dos(self):
        ...
        dos = self.interpolater.get_dos([10, 10, 10], emin=-10, emax=10,
                                        width=0.075)
        np.testing.assert_array_almost_equal(dos, self.correct_dos)

        # test normalising the DOS
        norm_dos = self.interpolater.get_dos([10, 10, 10], normalize=True)
        np.testing.assert_array_almost_equal(norm_dos, self.correct_norm_dos)
