...
class AmsetTest(unittest.TestCase):
    ...
    def test_parabolic_bands(self):
        ...
        for T in temperatures:
            N_c = (
                2
                * (2 * np.pi * mass * 9.11e-31 * 1.3806e-23 * T / ((6.626e-34) ** 2))
                ** 1.5
            )
            ...
            diff = abs(
                np.array(amset.mobility["n"]["ACD"][c][T])
                - np.array(amset.mobility["n"]["SPB_ACD"][c][T])
            )
            avg = (
                amset.mobility["n"]["ACD"][c][T] + amset.mobility["n"]["SPB_ACD"][c][T]
            ) / 2
    ...
    def test_GaAs_anisotropic(self):
        ...
        for mu in expected_mu.keys():
            self.assertLessEqual(
                np.std(  # GaAs band structure is isotropic
                    amset.mobility["n"][mu][-2e15][300]
                ),
                0.08 * np.mean(amset.mobility["n"][mu][-2e15][300]),
            )
            self.assertLess(
    ...
    def test_InP_isotropic_E(self):
        ...
        for mu in expected_mu.keys():
            ...
            self.assertLessEqual(
                abs(amset.mobility["n"][mu][-2e15][300][0] / expected_mu[mu] - 1), 0.02
            )
